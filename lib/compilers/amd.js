'use strict';
var AmdCompiler, EsLinter, JsHinter, NG_FACTORIES, UglifyJSOptimizer, _, _compileComponentFile, _compileIndex, _processComponent, _writeHTML, _writeMainData, _writeMainFile, anymatch, beautify, builder, defaultFactories, defaultOptions, factoryParse, fcache, fs, hasProp, log4js, logger, mkdirp, ngFactory, readComponents, ref, removeStrictOptions, sysPath, writeData;
...
_writeMainFile = function(config, options, done) {
  var index, iterate, keys, length, localOptions, pathBrowserify, paths, source, srcPath, template, tplOpts, types;
  paths = options.paths;
  ...
  srcPath = localOptions.mainTemplate || sysPath.resolve(__dirname, '../../templates/main.js');
...
module.exports = AmdCompiler = (function() {
  ...
  AmdCompiler.prototype.onCompile = function(generatedFiles, changedAssets) {
    ...
    options = _.pick(this, ['paths', 'lastPackages', 'options', 'config', 'optimizer']);
    resolve = function() {};
    reject = function() {};
    done = function(err) {
      resolve();
