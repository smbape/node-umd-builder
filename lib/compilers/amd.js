'use strict';
var AmdCompiler, EsLinter, JsHinter, NG_FACTORIES, UglifyJSOptimizer, _compileComponentFile, _compileIndex, _processComponent, _template, _writeHTML, _writeMainData, _writeMainFile, anymatch, beautify, builder, clone, cloneDeep, defaultFactories, defaultOptions, defaults, defaultsDeep, extend, factoryParse, fcache, fs, hasProp, isEqual, isFunction, isObject, log4js, logger, merge, mkdirp, modules, ngFactory, pick, readComponents, ref, removeStrictOptions, sysPath, writeData;
...
_compileComponentFile = function(path, component, config, memo, isAbsolutePath, options, opts, done) {
  var absolutePath, configPaths, deps, destFile, exports, extname, groupIndex, name, pathext, paths, plugin, processed, shim;
  name = component.name;
...
_writeMainFile = function(config, options, done) {
  var dirname, filename, imports, index, iterate, keys, length, localOptions, pathBrowserify, paths, source, template, tplOpts, types;
  paths = options.paths;
  config = clone(config);
...
_compileIndex = function(config, options, done) {
  var destFileSingle, e, err, paths, source, srcpath, stats, template, tplOpts;
  paths = options.paths;
  ...
  try {
    ...
    _writeHTML(template(defaults({
      ...
      return _writeHTML(template(defaults({
        build: 'web'
...
module.exports = AmdCompiler = (function() {
  ...
  function AmdCompiler(config) {
    ...
    this.options = options = merge({}, defaultOptions, (ref1 = config.plugins) != null ? ref1.amd : void 0);
  ...
  AmdCompiler.prototype.compile = function(params, done) {
    var comData, data, dst, err, locals, map, modulePath, name, parsed, path, ref1, umdData;
  ...
  AmdCompiler.prototype._compilePackages = function(generatedFiles, changedAssets) {
    ...
    for (dirname in packages) {
      ...
      if (!lastPackages || !isEqual(lastPackages[dirname], paths)) {
        hasChanged = true;
