'use strict';
var AmdCompiler, JsHinter, NG_FNS, NG_PREFIX, UglifyJSOptimizer, _, _compileComponentFile, _compileIndex, _processComponent, _writeHTML, _writeMainData, _writeMainFile, anymatch, beautify, builder, comWrapper, fcache, fs, hasProp, log4js, logger, mkdirp, ngFactoryProxy, ngModuleFactoryProxy, parse, reactFactoryProxy, readComponents, ref, removeStrictOptions, sysPath, umdWrapper, writeData,
...
_compileIndex = function(config, options, done) {
  var destFileSingle, err, error, paths, source, srcpath, template, tplOpts;
  paths = options.paths;
...
module.exports = AmdCompiler = (function() {
  ...
  AmdCompiler.prototype._compilePackages = function(generatedFiles, changedAssets) {
    var absPath, arg, args, basename, content, deps, dirname, hasChanged, hasFile, i, j, k, keys, lastPackages, len, len1, match, packageName, packages, path, paths, plugin, ref1, ref2, status;
    plugin = this;
    ...
    hasChanged = false;
    ref1 = Object.keys(packages).sort();
    for (j = 0, len = ref1.length; j < len; j++) {
      ...
      if (!lastPackages || !_.isEqual(lastPackages[dirname], paths)) {
        ...
        for (k = 0, len1 = paths.length; k < len1; k++) {
          path = paths[k];
          hasFile = true;
          ref2 = path.match(/([^\/\\]+)(?:\.[^\.]+)?$/), match = ref2[0], basename = ref2[1];
          arg = 'arg' + i;
          args.push(arg);
          deps.push('./' + basename);
