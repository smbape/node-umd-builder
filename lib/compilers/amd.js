...
module.exports = AmdCompiler = (function() {
  ...
  AmdCompiler.prototype.onCompile = function(generatedFiles, changedAssets, done) {
    var config, count, give, options, plugin, take, watcherIsReady;
    ...
    give = function(err) {
      if (--count === 0 || err) {
        ...
        _writeMainFile(config, options, function(err) {
          ...
          if (watcherIsReady) {
            builder.bwatcher.watcherIsReady = true;
          }
  ...
  AmdCompiler.prototype._compilePackages = function(generatedFiles, changedAssets) {
    var absPath, arg, args, basename, content, deps, dirname, hasFile, i, j, keys, lastPackages, len, match, packageName, packages, path, paths, plugin, ref1, status;
    plugin = this;
