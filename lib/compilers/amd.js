...
factoryParse = require("umd-loader/lib/method-parser").parse;
...
(function() {
  var j, len, name, ref;
  ref = ["usable", "run", "config", "module", "factory", "filter", "directive", "controller", "service", "value", "constant", "decorator", "provider"];
  for (j = 0, len = ref.length; j < len; j++) {
    name = ref[j];
...
module.exports = AmdCompiler = (function() {
  ...
  function AmdCompiler(config) {
    var Ctor, EsLinter, JsHinter, ref;
    if (config == null) {
      config = {};
    }
  ...
  AmdCompiler.prototype.compile = function(params, done) {
    var _UNUSED_, comData, data, dst, err, joinTo, map, modulePath, name, parsed, path, ref, umdData;
    joinTo = this.joinTo;
    ...
    if (!this.isIgnored(params.path)) {
      try {
        ref = parsed = factoryParse(data, this.parseOptions), _UNUSED_ = ref[0], name = ref[1];
      } catch (error) {
        err = error;
        logger.error(err);
      }
    ...
    this._getComponents((function(_this) {
      return function(err, components) {
        var match, ref1, relpath;
        if (err) {
          done(err);
          return;
        }
        if (/^bower_components[\/\\]/.test(path) && _this.isVendor && _this.isVendor(path)) {
          ref1 = path.match(/^bower_components[\/\\]([^\/\\]+)[\/\\](.+)/), match = ref1[0], name = ref1[1], relpath = ref1[2];
  ...
  AmdCompiler.prototype._processPackages = function(plugin, name, components) {
    var _UNUSED_, __deepackName, __packageName, _dirname, _module, _packageName, absPath, basename, content, deps, dirname, getIndex, hasChanged, hasFile, i, j, k, key, lastIndex, lastPackages, lastPackagesKey, len, len1, match, obj, packageName, packages, path, paths, ref, ref1, reg, relModulePath, relpath, sorter, status;
    ...
    for (dirname in lastPackages) {
      ...
      for (j = 0, len = paths.length; j < len; j++) {
        path = paths[j];
        ref = path.match(/([^\/\\]+)(?:\.[^\.]+)?$/), _UNUSED_ = ref[0], basename = ref[1];
    ...
    getIndex = function(path) {
      var file, files, i, k, len1, ref1;
      ref1 = dirname.match(/^bower_components[\/\\]([^\/\\]+)/), _UNUSED_ = ref1[0], name = ref1[1];
    ...
    for (dirname in packages) {
      ...
      for (k = 0, len1 = paths.length; k < len1; k++) {
        ...
        ref1 = sysPath.parse(relpath), basename = ref1.name, _dirname = ref1.dir;
        if (basename === __packageName || basename === __deepackName) {
          continue;
        }
