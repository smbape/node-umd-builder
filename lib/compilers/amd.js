...
module.exports = AmdCompiler = (function() {
            ...
            if (!_this.isVendor || !_this.isVendor(path)) {
              ...
              while (match = reg.exec(path)) {
                ...
                if (_this.canJoin(dirname + "/" + deepackName, joinTo)) {
                  ...
                  _this.deepacks[dirname][pathWithoutExt] = true;
                }
              }
              if (!hasProp.call(_this.packages, dirname)) {
                _this.packages[dirname] = {};
              }
              _this.packages[dirname][pathWithoutExt] = true;
            }
            done(err, {
              data: comData,
              path: path
  ...
  AmdCompiler.prototype.onCompile = function(generatedFiles, changedAssets) {
    var config, count, done, give, options, plugin, resolve, take;
  ...
  AmdCompiler.prototype._lint = function(options, done) {
    ...
    if (linter = this.linter) {
      ...
      if ((ref1 = sysPath.basename(path)) === fcache.packageName.slice(0, -".js".length) || ref1 === fcache.deepackName.slice(0, -".js".length)) {
        done(null, options);
        return;
      }
      linter.lint({
  ...
  AmdCompiler.prototype._compilePackages = function(generatedFiles, changedAssets) {
    ...
    res1 = this._processPackages(plugin, "package");
    if (!plugin.options.deepack) {
      return res1;
    }
    res2 = this._processPackages(plugin, "deepack");
    return res1 || res2;
  };

  AmdCompiler.prototype._processPackages = function(plugin, name) {
    var __deepackName, __packageName, _dirname, _module, _packageName, absPath, basename, content, deps, dirname, hasChanged, hasFile, i, j, k, key, lastIndex, lastPackages, lastPackagesKey, len, len1, match, obj, packageName, packageNameWithoutExt, packages, path, paths, ref1, ref2, reg, relModulePath, relpath, status;
