var EXPRESSION_REG, TRF_DICT, babylon, cid, clone, defaults, escodegen, extend, hasOwn, isDedugEnabled, isObject, lookupTransforms, parse, shiftRange, shiftTransform, strReplace, transform,
...
lookupTransforms = function(ast, transformations, state, astStack, stateStack) {
  var attribute, attributes, closingElement, currState, expression, i, iast, inExpression, len, prevInExpression, prop, ref, value;
  if (state == null) {
    state = {
      level: 0,
      flattern: []
...
TRF_DICT = {
  spRepeat: function(str, options, transformations, start, end, state, node) {
    ...
    if (state.attributes.indexOf("spShow") !== -1) {
...
(function() {
  ...
  delegate = function(type) {
    ...
    TRF_DICT["sp" + type] = function(str, options, transformations, start, end) {
      return str.slice(0, start) + "on" + type + str.slice(end);
    };
...
parse = function(str, options) {
  return babylon.parse(str, {
    plugins: ["jsx", "flow", "jsx", "doExpressions", "functionBind", "classConstructorCall", "exportExtensions", "dynamicImport", "classProperties", "decorators", "trailingFunctionCommas", "asyncFunctions", "exponentiationOperator", "asyncGenerators", "objectRestSpread"],
    sourceType: "module"
  });
};
