...
config = exports.config = {
  ...
  compilers: [require('./compilers/amd'), require('./compilers/copy'), require('./compilers/relativecss'), require('./compilers/csso')],
  modules: {
  ...
  initialize: function(config, done) {
    builder.buildBower(config, function() {
      read(config.paths.root, 'bower', function(err, components) {
        var component, i, len;
        if (err) {
          throw err;
        }
