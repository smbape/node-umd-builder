...
var _, _compileBowerFile, _patterMatchBowerFiles, _processBowerConfiguration, _processComponent, _processComponentMainFiles, _writeMainFile, anymatch, build, buildBower, buildClient, buildSem, capitalize, cluster, compileIndex, emptyFn, explore, exploreClientFiles, fs, initConfig, isFile, isFileSync, log4js, logger, mkdirp, semLib, sysPath, util, watchClientFiles;
...
_processComponentMainFiles = function(mainFiles, component, bowerConfig, config, options, done) {
  ...
  for (i = 0, len = mainFiles.length; i < len; i++) {
    ...
    if (/\*/.test(path)) {
      ...
      _patterMatchBowerFiles(component, bowerConfig, config, {
        path: path,
...
_patterMatchBowerFiles = function(component, bowerConfig, config, arg, options, done) {
...
_writeMainFile = function(config, options) {
  var MAIN_JS_FILE, bundles, mainjs, writer;
